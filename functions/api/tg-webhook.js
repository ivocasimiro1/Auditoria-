...
async function handleRanking(chatId) {
  const mes = curMes();
  const since = mes + '-01';
  ...
  for (const r of regs) {
    ...
    if (!byStore[r.store_id]) byStore[r.store_id] = { ok: 0, total: 0, gaps: 0, noTalao: 0 };
  ...
  if (!ranked.length) { await tgSend(chatId, `📊 Sem dados para ${mesLabel(mes)}`); return; }
  ...
  await tgSend(chatId,
    `📊 *RANKING ${mesLabel(mes).toUpperCase()}*\n\n${lines.join('\n')}\n\n✅ ${ok} lojas OK  🔴 ${bad} com problemas\nActualizado às ${hhmm()}`
  );
}

async function handleStatus(chatId, query) {
  const mes = curMes();
  const since = mes + '-01';
  const [regs, cfgs] = await Promise.all([
  ...
  if (!cfg) {
    await tgSend(chatId, `❓ Loja não encontrada: *${query}*\n\nUsa /ranking para ver todas as lojas.`);
    return;
  }
  ...
  if (!sr.length) { await tgSend(chatId, `🏪 *${empName}*\n\nSem dados para ${mesLabel(mes)}.`); return; }
...
export async function onRequestPost(context) {
  try {
    ...
    const text = msg.text.trim().replace(/@\w+$/, ''); // strip @botname suffix
